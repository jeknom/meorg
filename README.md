...
- **Generate executables for all platforms:** Execute the `create-bins.sh <output-dir>` script. It will generate executables for all platforms and then make copies in the provided path.
...
- Create symlink (at-least on Mac & Linux): `sudo ln -s <absolute-path-to-repo>/meorg/MeOrg/bin/Release/net10.0/<your-target-platform>/publish/MeOrg /usr/local/bin/meorg`
